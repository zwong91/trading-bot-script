/**
 * DLMM V2.2 BNB-USDC 流动性测试
 * 测试在BSC网络上使用DLMM Liquidity Book协议添加BNB-USDC流动性
 */
...
/**
 * 测试DLMM BNB-USDC流动性添加
 */
async function testDLMMBNBUSDCLiquidity(): Promise<void> {
    try {
        logger.info("🧪 开始测试 DLMM V2.2 BNB-USDC 流动性添加");
        ...
        logger.info("\n🎉 DLMM BNB-USDC流动性测试完成!");

    } catch (error) {
        logger.error("❌ DLMM BNB-USDC流动性测试失败:", error instanceof Error ? error.message : String(error));
        throw error;
    }
}
...
async function testBNBWrapping(): Promise<void> {
    try {
        ...
        logger.info("   3. 批准WBNB和USDC给DLMM路由器");
 ...
 * 显示DLMM流动性特点
 */
function displayDLMMFeatures(): void {
    logger.info("\n📚 DLMM V2.2 Liquidity Book 特点:");
...
async function main(): Promise<void> {
    try {
        logger.info("🚀 启动 DLMM V2.2 BNB-USDC 流动性全面测试");
        ...
        // 测试流动性添加
        await testDLMMBNBUSDCLiquidity();
